...
echo "🧹 Cleaning up any previous server sessions..."
PIDS_GRADIO=$(lsof -t -i :7861)
if [ ! -z "$PIDS_GRADIO" ]; then
...
if ! lsof -i :9868 > /dev/null; then
    # Fallback to absolute binary path
    echo "⚠️ 'pinchtab' command not directly in PATH, falling back to absolute binary path..."
    /Users/applem1pro/.local/state/fnm_multishells/83177_1779390468267/bin/pinchtab bridge --port 9868 > pinchtab_bridge.log 2>&1 &
    sleep 3
fi

if ! lsof -i :9868 > /dev/null; then
    echo "❌ Failed to start Pinchtab bridge. Please check pinchtab_bridge.log."
    exit 1
fi

echo "✨ Pinchtab bridge is running!"
