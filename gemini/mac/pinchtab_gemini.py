...
def get_pinchtab_token():
    ...
    # 2. Try to fetch from pinchtab CLI (Optimized for Mac)
    try:
...
class PinchtabGeminiClient:
    ...
    def wait_for_ready(self, timeout=30):
        print(f"⏳ [Mac] Waiting for Pinchtab Bridge at {self.base_url}...")
    ...
    def navigate(self, url, tab_id=None, new_tab=False):
        ...
        if resp.status_code == 401:
            raise Exception("401 Unauthorized: Pinchtab token is invalid. Run 'pinchtab security down' in Terminal.")
